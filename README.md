...
download, the disc list, the firmware list, the save negotiation. RomM answers
those with a 401 -- its signal to go and sign in, distinct from the 403 it gives
an account that simply lacks a scope. On a 401 the shell reloads the window and
lets RomM's own frontend take it from there, so you get the login your server
ships, and an OIDC server still goes to its provider.
...
launch of that game. Sign in and press Play again.
